    ...
    const likearticle = (i) => {
      ...
      useEffect(() => {
        ...
        for (let i = 0; i < likebookmarkarr.length; i++) {
          let likebox = document.getElementById(`likes_likebox${i}`);
                    ...
                    <li key={item.id} id="likes_item">
                            ...
                            <div style={{marginBottom: "35%" }}>
                                {
                                    ...
                                    <img id="likes_img2" src={item.userPicture} alt="" onClick={()=>gotoUserprofile(item.userId)}/> :
                                    <img id="likes_img2" src='../image/Profileimage.png' onClick={()=>gotoUserprofile(item.userId)}/>
                                }
                                <strong id="likes_userId" onClick={()=>gotoUserprofile(item.userId)}>{item.userId}</strong>
                                <strong id="likes_follow_button">팔로우</strong>
                            </div>
                            <div
                                id={`likes_likebox${index}`}
                                style={{
                                marginBottom: "5%",
                                position: "relative",
                                display: "flex",
                        ...
                        <Col className='likes_detail'>
                            <div id='likes_detail_form'>
                                <strong id="likes_cafeName" onClick={()=>gotoCafeprofile(item.cafeName)}>{item.cafeName}</strong>
                            </div>
                        </Col>

                        <Col style={{ width: "50px", height: "100%" }}>
                            <div style={{width: "100%", height: "100%", paddingTop: "3%", paddingBottom: "3%"}}>
                                <img id="likes_img3" src={item.file.filePath} onClick={()=>{gotoDetail(item.feedNo)}}/>
                            </div>
                        </Col>
                    </li>
                );
